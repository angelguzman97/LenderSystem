...
<script>
  $(document).ready(function() {
    ...
    $.each(data, function(index, res) {
      ...
      html += '<input type="hidden" id="id_prestamo" name="id_prestamo" value="'+res.id_prestamo+'">';
      ...
      html += '</div>';
       ...
       ///////Abonos del cliente//////
       html += '<divclass="form-group mt-2"><label for="abono" class="fw-bold">Abono: $</label>'+" "+'<input style="width: 65px;" id="abono" type="number" name="abono"></div>';
       
       ///////Fecha del pago del cliente//////
       html += '<divclass="form-group mt-2"><label for="fecha_pago" class="fw-bold"> Fecha del pago:</label>'+" "+'<input id="fecha_pago" class="form-control w-50" name="fecha_pago" value="<?php date_default_timezone_set('America/Mexico_City');
                                                                            setlocale(LC_TIME, 'es_MX');
                                                                            echo strftime('%d de %B %Y'); ?>" readonly></input></div>';

       ///////////div de todos los datos del pago del cliente////////////////////
       html += '</div>';
       
       ///////////Botones////////////////////
       html += '<div class="mt-2" style="text-align: center;"><button class="btn btn-danger btn-agregar-cuota mt-2 align-items-center" type="submit" data-id="' + res.id_cliente + '" >Agregar Cuota</button>'+" ";
