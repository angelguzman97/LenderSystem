...
class PrestamosModel extends Query
{
    ...
    private $porcentaje, $plazo, $cantidad_dia, $total_pago, $fecha_inicio, $fecha_final, $fecha_saldo, $estado, $id_cantidad, $id_cliente, $id_ruta, $id, $id_prestamo, $saldo, $saldo_vencido;
    ...
    public function registrarPrestamos(int $porcentaje, int $plazo, int $cantidad_dia, int $total_pago, string $fecha_inicio, string $fecha_final, int $id_cantidad, int $id_cliente, int $id_ruta){
        ...
        if (empty($existe)) {
            ...
            $sql = "INSERT INTO prestamos(porcentaje, plazo, cantidad_dias, total_pago, fecha_inicial, fecha_final, id_cantidad, id_cliente, id_ruta) VALUES(?,?,?,?,?,?,?,?,?)";
            ...
            $datos = array($this->porcentaje, $this->plazo, $this->cantidad_dia, $this->total_pago, $this->fecha_inicio, $this->fecha_final, $this->id_cantidad, $this->id_cliente, $this->id_ruta);
    ...
    public function registrarSaldo(int $saldo, int $saldo_vencido, string $fecha_saldo, int $id_cliente){
        ...
        $this->id_cliente = $id_cliente;
        $verificar = "SELECT * FROM saldos WHERE id_cliente = $id_cliente AND saldo_vencido=0";
        //Se ejecuta trayendo el método select que ya se tiene creado como parámetro se le pasa el parámetro $verificar y todo eso se almacena en una variable.
        $existe = $this->select($verificar);
        //Se erifica con un if. Si existe se hace la consulta
        if (empty($existe)) {
    ...
    public function modificarPrestamos(int $porcentaje, int $plazo, int $cantidad_dia, int $total_pago, string $fecha_inicio, string $fecha_final, int $id_cantidad, int $id_cliente, int $id_ruta, int $id)
     {
        $this->porcentaje = $porcentaje;
        $this->plazo = $plazo;
        $this->cantidad_dia = $cantidad_dia;
        ...
        $this->id_ruta = $id_ruta;
         ...
         $sql = "UPDATE Prestamos SET porcentaje = ?, plazo = ?, cantidad_dias = ?, total_pago = ?, fecha_inicial = ?, fecha_final = ?, id_cantidad = ? WHERE  id_cliente = ? AND id_ruta = ? AND id= ?";
 
         //Se crea una variable que contiene el array donde indicamos las variables creadas con los this y esa variable se enviará a la carpeta config/App/Query.php
         $datos = array($this->porcentaje, $this->plazo, $this->cantidad_dia, $this->total_pago, $this->fecha_inicio, $this->fecha_final, $this->id_cantidad, $this->id_cliente, $this->id_ruta, $this->id);
